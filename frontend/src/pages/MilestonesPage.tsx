import { CheckCircle, ExpandMore, Schedule, WarningAmber } from "@mui/icons-material";
import { Accordion, AccordionDetails, AccordionSummary, Box, Card, CardContent, Checkbox, Chip, Grid, LinearProgress, Stack, Typography } from "@mui/material";
...
export function MilestonesPage() {
  ...
  return (
    <Box sx={{ display: "grid", gap: 3 }}>
      <Box>
        <Typography variant="h4">Milestones & Tasks</Typography>
        <Typography color="text.secondary">Track milestones and tasks across every active app.</Typography>
      </Box>
              ...
              <Card key={app.id}>
                <CardContent sx={{ p: 0 }}>
                  ...
                  >
                    ...
                    <Typography variant="caption" color="text.secondary" sx={{ minWidth: 36, textAlign: "right" }}>{app.progress ?? 0}%</Typography>
                  </Box>
                  {milestones.map((m) => {
                    const tasks = m.tasks ?? [];
                          ...
                          {tasks.map((task) => (
                            <Stack key={task.id} direction="row" alignItems="center" sx={{ py: 0.5, px: 1 }}>
                              <Checkbox
                                ...
                                onChange={() => updateTask({ id: task.id, milestoneId: m.id, patch: { status: task.status === "DONE" ? "TODO" : "DONE" } })}
                              />
                              <Box sx={{ flex: 1 }}>
